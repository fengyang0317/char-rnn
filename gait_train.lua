...
require 'util.OneHot'
require 'util.misc'
local GaitLoader = require 'util.GaitLoader'
...
cmd = torch.CmdLine()
cmd:text()
cmd:text('Train a gait view model')
cmd:text()
cmd:text('Options')
-- data
cmd:option('-data_dir','/home/yfeng23/lab/dataset/gait/DatasetB/conv8/','data directory. Should contain the data')
-- model params
...
cmd:option('-val_frac',0.2,'fraction of data that goes into validation set')
...
cmd:option('-src', '090', 'the input angle')
cmd:option('-dst', '054', 'the output angle')
cmd:option('-seq_length',10,'max length')
cmd:text()
...
local loader = GaitLoader.create(opt.data_dir, opt.src, opt.dst, split_sizes)
...
if string.len(opt.init_from) > 0 then
    ...
    if opt.model == 'lstm' then
        protos.rnn = LSTM.lstm(loader:get_feat_size(), opt.rnn_size, opt.num_layers, opt.dropout)
